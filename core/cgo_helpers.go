...
// packSAtom reads sliced Go data structure out from plain C format.
func packSAtom(v []Atom, ptr0 *C.t_atom) {
	const m = 0x7fffffff
	for i0 := range v {
		ptr1 := (*(*[m / sizeOfAtomValue]C.t_atom)(unsafe.Pointer(ptr0)))[i0]
		v[i0] = *NewAtomRef(unsafe.Pointer(&ptr1))
	}
}
...
func listHookC08DCEF4(crecv *C.char, cargc C.int, cargv *C.t_atom) {
	if listHookC08DCEF4Func != nil {
		...
		var argvc08dcef4 []Atom
		packSAtom(argvc08dcef4, cargv)
...
// allocAtomMemory allocates memory for type C.t_atom in C.
// The caller is responsible for freeing the this memory via C.free.
func allocAtomMemory(n int) unsafe.Pointer {
	mem, err := C.calloc(C.size_t(n), (C.size_t)(sizeOfAtomValue))
	if err != nil {
...
const sizeOfAtomValue = unsafe.Sizeof([1]C.t_atom{})

const sizeOfPtr = unsafe.Sizeof(&struct{}{})

// unpackArgSAtom transforms a sliced Go data structure into plain C format.
func unpackArgSAtom(x []Atom) (unpacked *C.t_atom, allocs *cgoAllocMap) {
	if x == nil {
		return nil, nil
	}
	allocs = new(cgoAllocMap)
	defer runtime.SetFinalizer(&unpacked, func(**C.t_atom) {
		go allocs.Free()
	})

	len0 := len(x)
	mem0 := allocAtomMemory(len0)
	allocs.Add(mem0)
	h0 := &sliceHeader{
		Data: uintptr(mem0),
		Cap:  len0,
	...
	v0 := *(*[]C.t_atom)(unsafe.Pointer(h0))
	for i0 := range x {
		v0[i0], _ = x[i0].PassValue()
	}
	h := (*sliceHeader)(unsafe.Pointer(&v0))
	unpacked = (*C.t_atom)(unsafe.Pointer(h.Data))
	return
}
